...
namespace Oldmansoft.ApplicationService.MoneyBag.WebApp.Controllers
{
    public class WalletRechargeController : ApiController
    {
        ...
        {
            long transactionId;
            return new Application.Money().Recharge(id, model.ClientAppId, model.ClientOrderId, model.Cent, model.Description, model.Callback, out transactionId);
        }

        public Models.TradeResponseModel Post(Guid id, [FromBody]Models.TradeRequestModel model)
        {
            long transactionId;
            var rechargeResult = new Application.Money().Recharge(id, model.ClientAppId, model.ClientOrderId, model.Cent, model.Description, model.Callback, out transactionId);
